...
function obdelajDregljaj(socket) {
  socket.on('dregljaj', function (dregljaj) {
    if (dregljaj.vzdevek) {
      var socketIdNaslovnika = null;
      for (var id in vzdevkiGledeNaSocket) { //se sprehodu po trenutno prijavljenih ljudeh
       if (dregljaj.vzdevek == vzdevkiGledeNaSocket[id]) { // ce najde istoimenskega
         socketIdNaslovnika = id;                         // ga da za naslovnika
         break;
       }
      }
      if(socketIdNaslovnika && socketIdNaslovnika == socket.id) { //ce == posiljatelju si ne more poslat
        io.sockets.sockets[socket.id].emit('sporocilo', {
         besedilo: "Dregljaja ne moreš poslati samemu sebi!"});
      } else if(socketIdNaslovnika) {  //če !=, potem posljes
      io.sockets.sockets[socketIdNaslovnika].emit('dregljaj', { vzdevek: dregljaj.vzdevek, dregljaj: true});
     } else { //ce te osebe ni na seznamu uporabnikov
       io.sockets.sockets[socket.id].emit('sporocilo', {
